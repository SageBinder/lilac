...
/-! ## traverse -/
theorem Vec.pure_cons
  {m α n} [i : Applicative m] [LawfulApplicative m]
  {x : α} {xs : Vec α n}
  : i.pure (Vec.cons x xs) = Vec.cons <$> i.pure x <*> i.pure xs := by simp

theorem Vec.map_seq_inj
  {m α} [Applicative m] [LawfulApplicative m]
  {n} {x p : m α} {y q : m (Vec α n)}
  : Vec.cons <$> x <*> y = Vec.cons <$> p <*> q → x = p ∧ y = q
| h =>
  have x_eq_p := by

    sorry
  have y_eq_q := by sorry
  ⟨x_eq_p, y_eq_q⟩

theorem Vec.map_seq_inj_pure
  {m α} [i : Applicative m] [LawfulApplicative m]
  {n} {x : m α} {y : m (Vec α n)} {p : α}
  : {q : Vec α n} → Vec.cons <$> x <*> y = Vec.cons <$> pure p <*> pure q → x = pure p ∧ y = pure q
| #(), h => by
  simp at h
  have blah : pure (p :: #()) = cons <$> i.pure p <*> i.pure #() := by simp only [Nat.reduceAdd,
    map_pure, seq_pure]
  rw [blah] at h
  sorry
| z::zs, h => by
...
| n, y, ys, x, xs =>
  have forward h := by rw [pure_cons] at h ; unfold traverse at h ; exact map_seq_inj_pure h
  have backward := fun ⟨h1, h2⟩ ↦ by simp_all
  ⟨forward, backward⟩
